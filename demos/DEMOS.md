...
**That happened**, for one chapter, on the branch answering the owner's notes
[#75](https://github.com/zacharyelston/OurBubble/issues/75) and
[#76](https://github.com/zacharyelston/OurBubble/issues/76). *Two worlds threaded* carries three of
these stills; `tools/figures.mjs` is the small script that drives these modules under node and
writes them, and `make check` refuses a committed figure that is not what this code emits today —
in either direction, and refuses itself if that comparison stops biting. `ART_DIRECTION.md` now
names the two kinds of picture and the rule for retiring a study.

Nothing about **this** directory changed to make that work. The stills come out of the same
`stillFrom` the button uses, on the same drawings, at states the steps already declare; the one
thing the generator does to a drawing is drop the wireframe's "drag it, and the numbers are in the
table below" — true here, false in a file with no page around it.

The rest of the chapters are unconverted, and the choice of which beats earn a picture is still the
owner's rather than a quota.
