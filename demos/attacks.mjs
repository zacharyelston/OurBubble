...
export const ATTACKS = [
  ...
  { guard: "gate 3 · a beat number typed into a step", file: "steps.mjs",
    ...
    expect: "types a digit into the string" },
  { guard: "gate 3 · a beat id typed into a step", file: "steps.mjs",
    from: 'act: "Walk it."', to: 'act: "Walk it. (two-dots-and-a-line.6)"',
    expect: "types a digit into the string" },
  ...
  { guard: "gate 9b · a stale beat count in the page table", file: "DEMOS.md",
    from: "| Two dots, a line, and the first thing that closes | 10 | 9 |",
    to: "| Two dots, a line, and the first thing that closes | 11 | 9 |",
    expect: "beats, and steps.json derives" },
  { guard: "gate 9b · a page dropped out of the page table", file: "DEMOS.md",
    from: "| [`two-worlds-threaded.html`](two-worlds-threaded.html) | Two worlds threaded | 6 | 5 |",
    to: "| Two worlds threaded | 6 | 5 |",
    expect: "has no page-table row for" },
  { guard: "gate 9b · a page table row with its counts written out in words", file: "DEMOS.md",
    from: "| [`two-worlds-threaded.html`](two-worlds-threaded.html) | Two worlds threaded | 6 | 5 |",
    to: "| [`two-worlds-threaded.html`](two-worlds-threaded.html) | Two worlds threaded | six | five |",
    expect: "states no beat count and step count" },
  { guard: "gate 9b · a page table row claiming a step the page does not walk", file: "DEMOS.md",
    from: "| [`two-worlds-threaded.html`](two-worlds-threaded.html) | Two worlds threaded | 6 | 5 |",
    to: "| [`two-worlds-threaded.html`](two-worlds-threaded.html) | Two worlds threaded | 6 | 6 |",
    expect: "page walks 6 steps and it walks" },
...
${WIRE_CIRCLE}`,
  ...
  { guard: "gate 5 · the steps handed back in the wrong order", file: "core.mjs",
    from: "  const joined = steps.map((step, index) => {",
    to: "  const joined = [...steps].reverse().map((step, index) => {",
  ...
  // ── 5: a beat is its chapter's, and a step is labelled by its place on the page (issue #77) ───
  //
  // The label used to be the beat number, which is the thing that went stale three times. These are
  // the two ways the old shape could come back: a label that counts something outside the page, and
  // an id that is a bare number again.
  { guard: "gate 5 · a step labelled by a number instead of its place on the page", file: "core.mjs",
    from: "      label: `step ${index + 1} of ${steps.length}`,",
    to: "      label: `beat ${index + 35}`,",
    expect: "counts this page's steps and nothing outside the page" },
  { guard: "gate 5 · a step's label counting the book instead of the page", file: "core.mjs",
    from: "      label: `step ${index + 1} of ${steps.length}`,",
    to: "      label: `step ${index + 1} of 123`,",
    expect: "counts this page's steps and nothing outside the page" },
  { guard: "gate 5 · a beat id reduced to a bare number", file: "steps.json",
    from: '"beat": "two-worlds-threaded.1"', to: '"beat": "48"',
    expect: "is not this chapter's slug and a number" },
  ...
  { guard: "gate 11 · a probe cell moved somewhere else, leaving a constant unprobed",
    ...
    to: '        tag: "two-dots-closed", cells: [[0, 0], [0, 0]] },',
    expect: "is the same in every state" },
  { guard: "gate 11 · a column dropped out of a table's expectation", file: "core.test.mjs",
    from: '          return [name, term.text, term.refused ? "no" : "yes", printed(walk.running[index])];',
    to: '          return [name, term.text];',
    expect: "speaks for no column" },
  { guard: "gate 11 · the faces there are taken from another count", file: "steps.mjs",
    from: '                [[count(walked), String(cut.oct_faces),',
    to: '                [[count(walked), String(cut.mid_lines.length),',
    expect: "where the engine answers" },
  { guard: "gate 11 · a vendored never typed one table above the tried ticks",
    file: "steps.mjs",
    from: '                [[String(runaway.ticks), run.period === 0 ? "never" : String(run.period)]],',
    to: '                [[String(runaway.ticks), "never"]],',
    expect: "says something else" },
