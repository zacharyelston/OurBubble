...
The rest is what came back — from that world's plainer relatives, each named when it arrives, and
once from a small model that is not a wave at all. A ripple lopsided until one setting fixed it. What it costs to push on a
