# The shadow

> **Scope.** This is a book about a **toy**: a small world built inside a computer, and what happened
> when we pointed real tests at it. Nothing here is a claim about nature. Where a chapter uses a word
> like *vacuum* or *light cone*, the word names a pattern in the model.

<figure class="chapter-illustration">
  <img src="assets/shadow-question.svg" alt="A child and a small post cast long shadows across a curved field toward a glowing question mark: a visible shadow standing in for the unseen thing that cast it.">
  <figcaption><strong>Analogy — not data.</strong> A shadow is not evidence of its source — it is an invitation to build a test. That move, not the shadow, is what this book is about.</figcaption>
</figure>

A child stands in a yard at noon and her shadow is a stub around her feet. She comes back after
dinner and it is longer than she is, stretched across the grass toward the fence.

Nothing about that is strange. Everyone has seen it. What is worth noticing is that she has just
found something that *changes* — reliably, in the same direction, every day of her life — and that
she is under no obligation to do anything about it at all. Most of us do not.

But suppose she does. What is the smallest next thing she could possibly do?

## Two marks in the ground

She pushes a stick into the ground and scratches a line in the dirt where its shadow ends. An hour
later she comes back and scratches another.

Now there are two marks, and a gap between them, and the gap is a certain number of finger-widths
across. That number is not something she decided. She did not choose it, guess it, or prefer it. The
world put it there and she wrote it down.

That is the whole of what a measurement is. Not an instrument, not a talent, not a laboratory — just
something that changes, and someone willing to record by how much.

So she has a number. Does she have an answer?

## A number is not yet an answer

No. And it is worth being exact about why, because this is the gap the rest of the book lives in.

The two marks are compatible with almost anything. A sun that moves. A ground that tilts. A stick
that grows. She has no way to prefer one of those to the others, because nothing in the marks
themselves says which. A measurement on its own is a number with a time attached — true, and mute.

What turns it into an answer is a *reason to expect one thing rather than another*, written down
before you look. That is the harder half, it is the half people skip, and somebody did it with a
shadow a very long time ago.

Did he have to be a genius?

## What Eratosthenes did next

Around 240 BCE a man in Alexandria did the next thing, starting from almost exactly this.

Eratosthenes of Cyrene was the chief librarian there — he ran the largest collection of books in the
world, and he had the job because he was already eminent. A mathematician, a geographer, a poet.
Nobody was waiting on him to prove himself.

That is part of why it makes a good first example: you would not have needed to be him. The
measurement takes no new mathematics. There is no clever instrument in it. The arithmetic at the end
of it is one multiplication, and you could do it now, in your head, if you had the two numbers.

What he had instead of cleverness was a discrepancy he could not explain.

## Two posts, one shadow

The discrepancy was a story about a well. At Syene, far to the south, at noon on the longest day of
the year, sunlight was said to fall straight down a deep well and light the water at the bottom.
Straight down. An upright post standing beside that well threw no shadow at all.

In Alexandria, on the same day and at the same hour, an upright post *did* throw a shadow. A short
one, but a real one.

Two upright posts. Same day, same hour, same Sun. One casts a shadow; the other does not.

On a flat world that cannot happen. Upright posts on a flat world, lit by a Sun far enough away that
its rays arrive parallel, all lean their shadows the same way by the same amount — so if one of them
has no shadow, none of them do.

The ground between the two towns is therefore not flat. It curves. But by how much?

## How much it curves

Here is where he turned a shape into a number, and the number is what made the shape able to be
wrong.

He measured the angle of the Alexandria shadow, and it came out at about a fiftieth of a full turn.
That angle is doing all the work, because it is also the angle between the two towns as seen from the
centre of the Earth. If a fiftieth of the circle separates them, then the whole circle is fifty times
the distance between them.

He still needed that distance, and here the story is more ordinary than people expect. He did not
work it out; he looked it up. The figure in use for Alexandria to Syene was around five thousand
stadia. Where that figure came from, no ancient source says — the usual guess is the bematists,
professional pacers paid to walk between places and count their steps, because pacing was how a
kingdom knew its own size.

Fifty times five thousand. One multiplication, and he had a size for the Earth.

He was close. That is not the interesting part.

## The order he did things in

The interesting part is the order, because the order is the method and the method is the rest of this
book.

Notice what he did *not* do. He did not begin with a beautiful idea about spheres and then go hunting
for shadows that fitted it. He began with something he could not explain, proposed the smallest shape
that would explain it, and then committed to a number — a number that could have come out absurd,
and would have taken his shape down with it.

Four steps:

1. Notice something you cannot explain.
2. Propose the smallest structure that would explain it.
3. Say, in advance, what result would prove the proposal wrong.
4. Check. Keep the answer, especially when it is no.

Step three is the one people skip. It is uncomfortable, it slows everything down, and it is the only
step that makes the other three worth anything. Without it you have a story that fits — and stories
that fit are cheap, because you can always find one after you know the answer.

## Back to the child

Go back to her, and to the two marks in the ground.

She is one step away from Eratosthenes, and it is not a large step. She has something that changed
and a number for how much it changed. What she does not have is the thing step two and step three
would give her: a structure small enough to hold in her head, and a statement of what it would do
that could turn out to be false.

That is the hard part. It is also, conveniently, something you can build from nothing on the back of
an envelope — and the rest of this book is us building one, writing down what it should do before it
does it, and then letting it answer.

So we start with nothing at all, and see how little we need before something can happen.

*Each chapter ends with an italic line pointing at the files where its run lives, and the last
chapter is about going and looking.*

*What this chapter cites — and what it does not: [the simulations](the-simulations.md#s-the-shadow).*

**Next:** [Two dots, a line, and the first thing that closes](two-dots-and-a-line.md)—where you could
put a number, and the smallest thing that comes back to where it started.
