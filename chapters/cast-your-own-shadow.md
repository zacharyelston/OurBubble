...
Two of the smallest were made on a small wrapped copy of the Container — the same dots and the same
lines as the object you built, rather than one of its relatives: how often a walker stepping at
random along its lines comes home, and the resistance between two of its dots when every line is
made a resistor. The second is also written out as a standard circuit file that circuit simulators
read, so it can be checked in a language that owes nothing to this book.
