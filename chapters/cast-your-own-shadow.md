...
Clone the book — `github.com/zacharyelston/OurBubble` — and run these from its root. The first
command brings down the record the book quotes, at the one commit `record.lock` names; it needs read
access to the engine repository. The third needs [mdBook](https://rust-lang.github.io/mdBook/)
installed; the rest need only Python.
...
The second command is the book reading itself, and it is worth being exact about what it promises.
...
The third command builds the book — and building it **regenerates the appendix from the record**, so
the page you read is assembled rather than maintained. If the record has moved, the file changes under
you and `git status` says so. The fourth command reads the pages the build produced and follows every
