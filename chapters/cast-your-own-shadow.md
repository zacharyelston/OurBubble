...
Two of the smallest were made on wrapped copies of the Container — the same dots and lines as the
object you built, not one of its relatives: how often a walker stepping at random along its lines
comes home, the copies grown until the number stopped moving; and the resistance between two dots
when every line is made a resistor, which is also written out as a standard circuit file that
