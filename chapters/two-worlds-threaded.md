...
> **Scope.** A **toy**, and the last of it that fits on a napkin: two tetrahedra threaded through
> one another, and the same one rule. Every count and every table here is worked out while the book
...
Nothing so far in this book has cost anything to try. This one is the same: no new rule, no new
kind of number, no decision more interesting than *go on*.
...
them. The cube in that last line is not a new object to learn, either; it is only the box whose
eight corners the eight tips happen to land on, and the next chapter builds it properly.
...
Which leaves one thing to do with it. Make it move.
...
together, and you have already met the reason: the tick belongs to the shape. A step small enough
for a tetrahedron, and small enough for the shape between the tips, is too big for the two threaded.
That column is a limit rather than a setting — it is not the tick each shape likes, which is how
[make it move](make-it-move.md) used the phrase, but the size each one's tick has to stay under. The
third object's is under the tick we brought.
...
the row after it in twenty-fifths, and the one after that in hundred-and-twenty-fifths. The
denominator multiplies every tick, and a smaller tick multiplies it faster rather than slower.
...
running away gives you two rows before it fails that test. Two rows is exactly what the rule needs
...
started. It goes on, never quite repeating.
...
That is worth sitting with, because it is not what the last four chapters led you to expect. One
...
[the simulations](the-simulations.md#s-two-worlds-threaded).*
