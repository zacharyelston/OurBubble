...
| **Primers** | the owner's first-person scenes at the door of each movement, reflowed into the book's pacing after Kleist's *On the Marionette Theatre* (owner brief 2026-09-07: the book "is too abstract and vague and relies on too many small steps without constant resolidifying"); the drafter reflows the owner's words and adds no experience | `chapters/start-at-the-boundaries.md`, `chapters/staring-into-the-abyss.md`, their `OUTLINE.md` blocks, `EDITION_STANDARD.md` § *A primer*, `notes/primers-after-kleist.md` | one PR per primer or pair | **PR #102 MERGED 2026-09-07** at `92de3e9` on the owner's word after a round-1 proof-read (1 blocker, applied); the two primers are chapters 1 and 5 of the reading order; three movement doors (the machine, the record's pivot, the exit) registered in the note and unwritten, because a primer's scene is the owner's to supply. Owner's next word: "re read it fully. We might even branch here for a new Kleisty version" — the whole-book re-read and the proposal are `notes/a-kleist-edition.md` (options A/B/C, a `pre-kleist` tag as the branch point, recommendation B on `main`); awaiting the owner's choice |
| **Pivot (tranche H)** | the one-rule promise narrowed to what the record did; each result chapter names the object its run was made on; the ✎ beats carry the project's registered guess, said where the answer is (depth B of `notes/a-kleist-edition.md`, owner's choice 2026-09-07) | `chapters/what-you-will-have.md`, `make-it-move.md`, `room-and-a-world-with-no-edge.md`, `the-round-ripple.md`, `the-bubble-and-its-bill.md`, `the-wall-that-worked-and-didnt.md`, `where-negative-energy-appears.md`, `a-number-without-the-answer-key.md`, `when-the-expected-law-fails.md`, their `OUTLINE.md` blocks, `edition.json` retired phrasings, one anchor in `demos/steps.mjs` | one PR | **PR #106 MERGED 2026-09-08** at `8356ec7` on the owner's word ("take it as is") after six fresh-reader rounds (round 6 CLEAR); the owner's two decisions recorded on the PR — the refusal chapter is retitled *The law we were sure of* (its own PR), and the plural "we" at the ✎ moments stays; PR #97 closed as superseded. The branch point before any of this is kept as the branch `pre-kleist` at `92de3e9` (the proxy refused a tag push) |
| **Record on the Container (tranche I)** | cite the runs the pinned record already holds on the Container, the object the reader builds (issue #107; C-6 of the UniForge campaign, UniForge #364): 5.4 Ising on the stella mesh in the answer-key chapter, 7.0/8.2/8.3 (the random-walk constant, the resistor view, the netlist) in the exit chapter's re-run beat, *Room*'s "plainer worlds" sentence narrowed to "most". No record bump: every rung exists at the pin; the cited-path footprint grows. 2.7, 5.6/5.7 and 7.1 deliberately not cited — the reasons are in `notes/tranche-i-container-rungs.md` | `chapters/a-number-without-the-answer-key.md`, `cast-your-own-shadow.md`, `room-and-a-world-with-no-edge.md`, their `OUTLINE.md` blocks, `edition.json` appendix sections, `record.lock` paths, `record/` | one PR | drafted 2026-09-08; fresh proof-read before the owner; merges on the owner's word. The wave and switch results still run where PR #106 says; the campaign (C-0 the dial, then C-1…C-5) is UniForge's |
