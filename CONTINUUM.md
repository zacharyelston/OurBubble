...
| **Pivot (tranche H)** | the one-rule promise narrowed to what the record did; each result chapter names the object its run was made on; the ✎ beats carry the project's registered guess, said where the answer is (depth B of `notes/a-kleist-edition.md`, owner's choice 2026-09-07) | `chapters/what-you-will-have.md`, `make-it-move.md`, `room-and-a-world-with-no-edge.md`, `the-round-ripple.md`, `the-bubble-and-its-bill.md`, `the-wall-that-worked-and-didnt.md`, `where-negative-energy-appears.md`, `a-number-without-the-answer-key.md`, `when-the-expected-law-fails.md`, their `OUTLINE.md` blocks, `edition.json` retired phrasings, one anchor in `demos/steps.mjs` | one PR | draft PR open 2026-09-07 at `756e9d8` after six fresh-reader rounds (round 6 CLEAR, all findings applied); full tier 0 green at every head; awaiting the owner's word. The branch point before any of this is kept as the branch `pre-kleist` at `92de3e9` (the proxy refused a tag push) |
