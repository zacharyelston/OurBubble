...
5. Is that really the whole law — nothing else, ever?  *(yes, for every world she builds by hand and for the machine's ripples; the rest of the record is the same bookkeeping of differences and sums on the objects each chapter names, with nothing tuned to help an answer along. Some chapters do something else with it, and each says so where it does)*
...
8. Can I ask the stopwatch question now?  *(yes — and this is the object, the Container, named because it holds everything the napkin taught her and has room in it. The record the next chapters read from is older than this construction, so its runs were made on plainer worlds like it, each named where it comes; the first question the record answers is whether a ring in such a world comes out round)*

## 12 · Is it round?

1. Speed is distance over time — which half do I still not have?  *(distance: the dial has not been set. The world the record poked is the Container's plainer relative — the same cubes cut into six tetrahedra, no two-colouring, no twist, and for this run edges, so it finishes before the echo — whose grain is three kinds of line rather than three kinds of place; the dial has the same job on both)*
...
3. ✎ *Before we look:* if I give every line the same weight, will the ring be round?  *(and which way — along the cube's edges, or corner to corner through its middle? Ours are in the record from before either run, and neither said which way)*
...
1. Now that it behaves evenly — what would it cost to move a *piece* of it, contents and all?  *(the pivot, said once as the questions widen: what changes is the object under the question — she has met the ripple's relative already; the record's other runs were made on a plain grid of dots, a flat sheet of triangles, a line of dots, and a different small model, each named where its run begins; what does not change is the work, and the dial every one of these objects carries, fixed by the object or read from it. This chapter's object is a grid of dots and a formula that returns a number for a shape — the bubble)*
...
5. Now the blob inside — does it move differently?  *(exactly as far as before — which is what we had written down, beside the isolation we expected)*
