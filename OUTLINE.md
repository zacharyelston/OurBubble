...
**The grain, decided once (2026-09-02):** a beat is 100–200 words, hard ceiling 220, no hard floor.
Any section over 200 is listed in the PR with its one-idea reason; the excess does not migrate
between chapters. **✎ beats are exempt from the band by design** — a prompt to write your guess down
is supposed to be short. Any other section under 100 is listed with its reason the same way. Every
`##` section carries a `<!-- beat slug.n -->` marker — the chapter's own slug and the beat's place
inside that chapter, counted from 1, capped at twelve (issue #77). A chapter's opening prose carries
one only when it is the prose that answers a beat — `tools/beat_coverage.py` reads a marker there as
a declared excusal and otherwise skips the preamble, so nothing is ever inferred from a count.
...
3. What is the road?  *(first a puzzle, then why measuring gets done at edges; then the shadow, and what everyone else did with one; then a run of chapters on a napkin; then the napkin visibly running out, and the same one rule handed to a machine; then what came back — from plainer worlds like it, each named when it arrives, and once from a small model of switches, not ripples — and what it refused, kept on purpose)*
...
5. Is this book a puzzle?  *(it describes one — a small world built a piece at a time — and like every puzzle it goes together one way; she will be told where a hand had to choose. We start with the boundaries: the next page, named and linked, and an egg)*
...
3. So where am I standing when I measure?  *(at a boundary, always — the thing's or the instrument's, usually both. Nearest first: the chapter on why sharper shadows took two thousand years reads an edge in a different sense of the word — a limit rather than a place — the finest difference an instrument can still tell apart. Then, further off, several chapters of somebody reading an edge in the ordinary sense: a wall and what still gets past it, the narrow gap between two walls of another kind and what stops fitting there, a shaped push and where its bill is charged. The point is where somebody was standing, not what came back: in every one of them, somebody is at an edge reading it)*
...
3. What has Nothing to do with counting?  *(zero made the counting in this book possible. Write a 0 and you have one number — so there is one thing on the page, a 1; the zero and the one she counted are two things, and 2 is born. She wrote only the zero; the rest burst on to the scene)*
4. Does it stop?  *(no — there is no last mark once the zero is down; infinity is what she gets unasked out of the first mark, and it runs one way only: another mark can always be made and none unmakes the first. So the blank page the last chapter left her at is the right place to be: nothing is on it, and nothing is where everything she is about to count begins. Draw the first thing)*
...
3. Two dots and a line — can anything come back to where it started?  *(no: nothing closes yet)*
4. Add a third dot. What is the smallest thing that closes?  *(a triangle: three dots, three lines — and an inside)*
5. Put a number on each corner — how many differences do I have?  *(three, one per line)*
6. Walk around the triangle adding the differences — what do I get?  *(2 → 5 → 1 → 2: +3, −4, +1 = 0)*
7. Would any three numbers do that?  *(every time — because I came home)*
8. Why *exactly* zero and not nearly?  *(plus and minus signs cancel the way integers do)*
...
3. Four numbers on the corners, six differences on the lines: what does each face say?  *(walk around it: zero — four faces, four zeros)*
4. What if the six line-numbers came *first* — six arrows, not six differences?  *(then a face's loop sum needn't be zero: a number that belongs to the face — how much goes round it)*
5. And the four face-numbers around the inside — add them up the same way?  *(zero again: the outside of the inside closes. Coming home, one rung up)*
6. So how many kinds of number does this world have?  *(on dots, on lines, on faces, on the inside — and that is all there ever will be)*
...
4. So what does one tick *do* with those two rows?  *(one rule, the only one she runs by hand: each dot's number is pushed toward its neighbours' by the differences on its lines — and carries forward the motion the two rows already hold. The push sets the change in the change, not the change; that is the difference between a wave and a leak)*
5. Is that really the whole law?  *(yes, for every world she builds by hand and for the machine's ripples; the rest of the record is the same bookkeeping of differences and sums on the objects each chapter names, with nothing tuned to help an answer along. where a later chapter does something else with it — reads a formula off a shape, or puts switches where the numbers were — it says so where its object changes)*
...
9. Add the four up at every tick — what happens to the total?  *(it never changes: coming home, in time)*
10. Turn the dial — count one line for more — what changes?  *(the rhythm along that line; the total, and the average, never move)*  [napkin token `slosh_table_dialed`]
...
3. Count the new shape — and is there room in it?  *(six dots, twelve lines, eight faces. Its dots are the six middles, so they keep the lines' names — and three pairs of them are joined by nothing at all: the first two places in this book that are not neighbours)*  [napkin token `octa_counts`]
...
3. Make it move, at the tick that has worked all along.  *(it will not. The tick is too big for this object, and instead of sloshing the numbers run away — past a hundred million by tick twenty)*  [napkin token `stella_refusal`]
4. Then pick a smaller tick?  *(you can, and then the table cannot be written down: every tick that does hold gives two rows at most, and at no tick that can be written as a fraction does this world ever come home)*
...
8. Can I ask the stopwatch question now?  *(yes — and this is the object, the Container, named because it holds everything the napkin taught her and has room in it — the world the machine builds. The record the next chapters read from is older than it; most of its runs were made on plainer worlds like it, each named where its run begins, and those made on it say so. The first question it answers is whether a ring in such a world comes out round)*

## 12 · Is it round?

1. Speed is distance over time — which half do I still not have?  *(distance: the dial has not been set. The world the record poked is the Container's plainer relative — the same cubes cut into six tetrahedra instead of two-coloured, so its grain is three kinds of line where the Container's is three kinds of place; for this run it kept its edges and finished before the echo; the dial has the same job on both)*
...
3. ✎ *Before we look:* if I give every line the same weight, will the ring be round?  *(and which way — along the cube's edges, or corner to corner through its middle? Ours, one for this setting and one for the next, are in the record from before the run; neither said which way, so this half is hers alone)*
...
7. Could a machine decide that without my thumb on the scale?  *(on the same relative, wrapped this time: hand it the timings and a written menu of laws; it picks, and says by how much)*  [picture: the fit, the runner-up]
...
6. Is it a shell or a ring?  *(a ring around the waist — six times the front and back)*
...
2. ✎ *Before we look:* if I cut a region off from everything else, will what's inside get easier to push?  *(ours are in the record from before the run, on all three counts; the next beats say what they were)*
3. First: does the wall even isolate?  *(on the ripple's world, the cube cut into six tetrahedra: half a million times weaker inside)*  [picture: the fading wave]
4. How do I know I didn't just build a machine that damps everything?  *(the outside-to-outside path barely touched: 0.86)*
5. Now the blob inside — does it move differently?  *(as far as before, to within a millionth, where identical was written — the registered test missed by that millionth and rewritten after the run, the miss on the record; the isolation held; the run names the millionth as a reflected wave)*
...
5. ✎ *Before we look:* as the walls approach, should the difference grow or shrink?  *(ours is in the record from before the run, on both counts — the sign, and which way it moves — and the next beat says what it was)*
6. What did the little world do?  *(on a line of dots with a wall at each end — the smallest object the question fits, the same bookkeeping on it. We had written down below the line at every gap, deeper as the walls close, and the walls pulled together; it is below the line everywhere, deeper as the gap narrows — the walls pull together)*  [picture: the curve]
...
4. ✎ *Before we look:* should I trust an instrument on the unknown before it has recovered a known?  *(ours is in the record from before the run: margins for the known case's numbers, written down first)*
...
9. And on the world she built?  *(the same switches on the Container's dots, the same machine flipping them by chance; for this run the world wrapped in two directions and open in the third, and the record registered that the open ends would move one of the object's own numbers and none of the class's. The critical temperature was registered first as a range drawn around a first look; in the model's units it came out at 11.638, inside the range, against the block's 4.51; the two class numbers the block gave came back within 3.5% and 0.6% of the class values, inside their registered margins. The object's numbers moved, the class's did not — universality checked on her own object)*  [no stella-specific figure in the record (0504's eval says so) — the numbers are declared in the appendix]
...
3. ✎ *Before we look:* the whole-cavity rule is a square root — should a partial lump obey it too?  *(ours is in the record from before the run: the textbook law registered as the target — sure enough for that, not sure enough to bet on it — with a note beside it that a first look had suggested it would fail)*
4. What came back?  *(the registered −½ against p = −0.2753 — not a near miss, and outside the registered margin; the law is refused)*  [picture: the two slopes]
5. Why — and why is the explanation written as a *new* prediction rather than an excuse?  *(the lump is a fraction of the volume — a reason that was in the file with the warning before the run, and is still not a test of itself; if it is the reason, a bigger lump moves p toward −½ — a new commitment)*
6. What does a refusal tell us, coming from the same kind of instrument — a fitter told nothing — that found the ripple's law, the vacuum's coefficient and the class numbers?  *(that its yeses meant something: it refused a law its owners registered as the target, by a margin they wrote down first. Not the same code each time; the same discipline: no answer supplied)*
...
4. What is the check that would actually catch us?  *(re-run a test at full size, let it overwrite the data the book quotes, ask git. Two of the smallest were made on a small wrapped copy of the Container, the same dots and lines: a random walk's return, and the resistance between two dots with every line a resistor — the latter also a standard circuit file circuit simulators read)*  [rungs 7.0, 8.2, 8.3 in the appendix; no number quoted]
...
So beat what-you-will-have.1 is the moves themselves — make a mark, take the difference, add the differences up, see
whether they come home — with the instruments as better sticks, and the child differing from the
laboratory only in age and equipment. Beat what-you-will-have.2 hands her the things, and then the part worth more than
the things: the sciences whose papers look like a wall are doing those same moves in their own
notation, so she will recognise what they are doing without knowing their subjects. Beat what-you-will-have.4 closes on
...
beat what-you-will-have.2's recognition promise with it, because no chapter here teaches another science: that the
...
promise this book is not allowed to make (a proofreader, round 1, as a blocker). **The chapter
argues in one register and one only: where the account of a measurement is kept.** Round 1 refused
the thesis "anything you ever learn about what is inside has to come out through there"; round 2
found the same claim one beat further on ("because there is no other way out"), which is this
repository's own known failure — a blocker moved rather than fixed. So the sweep is the whole
chapter, the front door's pointer into it and the appendix note beside it: nothing in them says what
the world has to do, cannot do, or has no other way of doing. The three retired sentences are
...
page nothing in the edition would have stopped it. Rounds 3 and 4 each found the claim again in a
shape the last pattern did not cover — the modal deleted and the universal left standing ("Whatever
reached you got to you by crossing"), then an exclusivity claim written in synonyms ("the whole of
what is on offer to look at"); both are retired by name now, and round 5 read the chapter and found
the register clean.

**What the patterns actually key on, and what they do not.** Each one needs a *named* trigger in one
sentence beside a crossing or a boundary: a modal (`has to`, `must`, `cannot`, `never`, `no other`,
`the only`), or an exclusivity word (`only`, `solely`, `exclusively`, `except`, `nowhere else`,
`nothing but`), or an impossibility adjective. It is a denylist and it behaves like one: a sentence
that carries the idea with none of those words still passes — round 5 walked "The world has only one
...
*Drafter's note on chapters 6–8 (owner, 2026-09-02, tranche D). Three decisions, and the beats are
...
chapter 3 now, immediately after *The shadow*, and it is doing a different job: the reader has met
