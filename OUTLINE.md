...
9. And on the world she built?  *(the same switches on the Container's dots, the same machine flipping them by chance; for this run the world wrapped in two directions and open in the third — switches do not ripple, so an end is not an echo; the record registered that it would move one number of the object's own, the height at which its size-curves cross, and none of the class's, and it did. The critical temperature was registered first as a range drawn around a first look; in the model's units it came out at 11.638, inside the range, against the block's 4.51; the two class numbers the block gave came back within 3.5% and 0.6% of the class values, inside their registered margins. The object's numbers moved, the class's did not — universality checked on her own object)*  [no stella-specific figure in the record (0504's eval says so) — the numbers are declared in the appendix]
