...
8. Can I ask the stopwatch question now?  *(yes — and this is the object, the Container, named because it holds everything the napkin taught her and has room in it. The record the next chapters read from is older than this construction, so its runs were made on plainer worlds like it, each named where its run begins; the first question the record answers is whether a ring in such a world comes out round)*
...
3. ✎ *Before we look:* if I give every line the same weight, will the ring be round?  *(and which way — along the cube's edges, or corner to corner through its middle? Ours are in the record from before the run, and neither said which way)*
...
5. Now the blob inside — does it move differently?  *(exactly as far as before — the null we had written down as identical, beside the isolation we expected; it came back a millionth off, and the run says why that is not inertia)*
...
3. ✎ *Before we look:* the whole-cavity rule is a square root — should a partial lump obey it too?  *(ours is in the record from before the run: the textbook law registered as the target, with a note beside it that a first look had suggested it would fail)*
